...
JSObject *BuildObject(JSContext *cx, JSClass *classp = NULL, JSFunctionSpec *funcs = NULL, JSPropertySpec *props = NULL, void *priv = NULL,
                      JSObject *proto = NULL, JSObject *parent = NULL);
...
#define EMPTY_CTOR(name)                                                                                                                                       \
    JSBool name##_ctor(JSContext *cx, uintN argc, jsval *vp) { THROW_ERROR(cx, "Invalid Operation"); }
...
#define JSCLASS_DEFAULT_WITH_CTOR(ctor)                                                                                                                        \
    JSCLASS_SPEC(JS_PropertyStub, JS_PropertyStub, JS_PropertyStub, JS_StrictPropertyStub, JS_EnumerateStub, JS_ResolveStub, JS_ConvertStub, JS_FinalizeStub,  \
                 ctor)
...
template <typename T> inline JSBool __fastcall JS_ArrayToVector(JSContext *cx, JSObject *arr, std::vector<T> &vec) {
...
template <typename T> inline JSObject *__fastcall JS_VectorToArray(JSContext *cx, std::vector<T> &vec) {
    Converter<T> converter;
    size_t len = vec.size();
    ...
    auto end = vec.end();
    int i = 0;
    for (auto it = vec.begin(); it != end; it++, i++)
        jsarr[i] = converter(cx, vec[i]);
