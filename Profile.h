...
class Profile {
  protected:
    ...
    void init(ProfileType pt, char *_ipUsername, char *_password, char *_charname, char *_gateway, char _diff, unsigned int _maxLoginTime,
  ...
  public:
    ...
    Profile() { init(Vars.szProfile); }

    // Get profile by name
    Profile(char *profileName) { init(profileName); }

    // Create single player or tcp/ip host profile
    Profile(ProfileType pt, char *charName, char _diff) { init(pt, "", "", charName, "", _diff, 5000, 5000); }

    // Create battle.net or open battle.net profile
    Profile(ProfileType pt, char *account, char *_pass, char *_charname, char *_gateway) { init(pt, account, _pass, _charname, _gateway, 0, 5000, 5000); }

    // Create tcp/ip join profile
    Profile(ProfileType pt, char *_charname, char *ip) { init(pt, ip, "", _charname, "", 0, 5000, 5000); }
