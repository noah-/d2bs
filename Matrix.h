...
template <class TYPE, class ARG_TYPE> BOOL CMatrix<TYPE, ARG_TYPE>::Create(int cx, int cy, ARG_TYPE initValue) {
    if (!Create(cx, cy))
        return FALSE;
...
template <class TYPE, class ARG_TYPE> void CMatrix<TYPE, ARG_TYPE>::Destroy() {
    if (m_ppData) {
        for (int i = 0; i < m_cx; i++) {
            if (m_ppData[i]) {
                delete[] m_ppData[i];
...
template <class TYPE, class ARG_TYPE> BOOL CMatrix<TYPE, ARG_TYPE>::IsValidIndex(int x, int y) const {
...
template <class TYPE, class ARG_TYPE> BOOL CMatrix<TYPE, ARG_TYPE>::SetAt(int x, int y, ARG_TYPE data) {
    if (!IsValidIndex(x, y))
        return FALSE;
