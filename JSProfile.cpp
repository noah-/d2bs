...
CLASS_CTOR(profile) {
    ...
    try {
        ...
        // Profile(ProfileType.singlePlayer, charname, diff)
            ...
            prof = new Profile(PROFILETYPE_SINGLEPLAYER, JS_EncodeString(cx, JSVAL_TO_STRING(JS_ARGV(cx, vp)[1])), (char)(JSVAL_TO_INT(JS_ARGV(cx, vp)[2])));
        }
        // Profile(ProfileType.battleNet, account, pass, charname, gateway)
        ...
        // Profile(ProfileType.tcpIpHost, charname, diff)
            ...
            prof = new Profile(PROFILETYPE_TCPIP_HOST, JS_EncodeString(cx, JSVAL_TO_STRING(JS_ARGV(cx, vp)[1])), (char)(JSVAL_TO_INT(JS_ARGV(cx, vp)[2])));
        }
        // Profile(ProfileType.tcpIpJoin, charname, ip)
